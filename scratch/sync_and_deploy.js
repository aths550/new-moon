...
async function main() {
  ...
  console.log('Syncing indexer blocks and waiting for unshielded tNIGHT balance...');
  const syncedState = await rx.firstValueFrom(
    walletProvider.wallet.state().pipe(
      rx.tap((s) => {
        const balances = s.unshielded.balances || {};
        console.log(`Indexer block sync progress: unshielded balances = ${JSON.stringify(balances)}`);
      }),
      rx.filter((s) => {
        const coins = s.unshielded.availableCoins || [];
        return coins.length > 0 || Object.keys(s.unshielded.balances || {}).length > 0;
      }),
    ),
  );

  console.log('Unshielded funds found! Generating DUST...');
  await generateDust(logger, seed, syncedState.unshielded, walletProvider.wallet);
  ...
  const providers = {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'preprod-auction-store-sync',
      signingKeyStoreName: 'preprod-auction-signing-keys-sync',
