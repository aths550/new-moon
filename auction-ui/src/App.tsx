...
const App: React.FC = () => {
  ...
  const [auctionPhase, setAuctionPhase] = useState<'Commit' | 'Reveal' | 'Ended'>('Commit');
  ...
  const [statusSeverity, setStatusSeverity] = useState<'info' | 'success' | 'warning' | 'error'>('info');

  const connectWallet = useCallback(async () => {
    setWalletStatus('connecting');
    try {
      const midnight = (window as unknown as Record<string, unknown>).midnight as
        | { mnLace?: { isEnabled: () => Promise<boolean>; enable: () => Promise<unknown> } }
        | undefined;
      if (midnight?.mnLace) {
        ...
        if (enabled) {
          ...
          setStatusMessage('Lace wallet connected on Midnight Preprod network!');
        } else {
        ...
        setStatusMessage('Lace Wallet extension not detected. Running in Interactive Demo Mode.');
      }
    } catch {
      setWalletStatus('demo');
      setStatusSeverity('info');
  ...
  const handleRevealBid = useCallback(() => {
    ...
    const currentAmount = myBid ? myBid.amount : BigInt(localStorage.getItem('auction_bid_amount') || '0');
  ...
  const isWalletActive = walletStatus === 'connected' || walletStatus === 'demo';
  const phaseColor = auctionPhase === 'Commit' ? '#7c4dff' : auctionPhase === 'Reveal' ? '#ff9100' : '#00e676';
  const PhaseIcon = auctionPhase === 'Commit' ? LockIcon : auctionPhase === 'Reveal' ? VisibilityIcon : CheckCircleIcon;
  ...
  return (
    <ThemeProvider theme={darkTheme}>
      ...
      >
        {/* Navigation Bar */}
        <AppBar position="static" sx={{ background: 'rgba(19, 25, 41, 0.8)', backdropFilter: 'blur(20px)' }}>
          <Toolbar>
        ...
        <Container maxWidth="md" sx={{ py: 4 }}>
          ...
          {statusMessage && (
            <Alert severity={statusSeverity} sx={{ mb: 3, borderRadius: 3 }} onClose={() => setStatusMessage('')}>
              {statusMessage}
            </Alert>
          )}
          ...
          {walletStatus === 'demo' && (
            <Alert severity="info" sx={{ mb: 3, borderRadius: 3 }}>
              ℹ️ Running in <strong>Interactive Demo Mode</strong>. To connect a live browser wallet, install the official{' '}
              <Link
            ...
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  ...
                  <Box>
                    ...
                    <Chip
                      ...
                      sx={{ mt: 0.5, bgcolor: `${phaseColor}22`, color: phaseColor, fontWeight: 600 }}
                    />
                  </Box>
                </Box>
                ...
                <Stack direction="row" spacing={1}>
                  <Button
                    size="small"
                    variant={auctionPhase === 'Commit' ? 'contained' : 'outlined'}
                  ...
                  <Button
                    size="small"
                    variant={auctionPhase === 'Reveal' ? 'contained' : 'outlined'}
                  ...
                  <Button
                    size="small"
                    variant={auctionPhase === 'Ended' ? 'contained' : 'outlined'}
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={4} sx={{ justifyContent: 'space-between' }}>
                <Box>
                  ...
                  <Typography variant="h3" sx={{ fontWeight: 700, color: '#00e5ff' }}>
                    {highestBid.toString()}{' '}
                ...
                <Box sx={{ maxWidth: 280 }}>
                  <Typography variant="caption" color="text.secondary" sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                    <ShieldIcon sx={{ fontSize: 16, color: '#00e676' }} /> Zero-Knowledge Privacy
                  </Typography>
                  <Typography variant="body2" sx={{ color: '#00e676', mt: 0.5, fontSize: '0.85rem' }}>
                    🔒 Only the running max ever hits the ledger during Reveal. Losing bids are never disclosed or recorded anywhere on-chain.
                  </Typography>
                </Box>
              </Stack>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                      ...
                      <Box>
                        <Typography variant="subtitle2" sx={{ fontWeight: 600 }}>
                          Your Local Sealed Bid: {myBid.amount.toString()} tNIGHT
                        </Typography>
                        <Typography variant="caption" color="text.secondary">
                          Commitment Hash: 0x{myBid.commitmentHashHex.slice(0, 24)}...
                        </Typography>
                      </Box>
                    </Box>
                    <Chip
                      ...
                      color={auctionPhase === 'Commit' ? 'primary' : auctionPhase === 'Reveal' ? 'warning' : 'success'}
                      size="small"
                    />
                  </Stack>
                </Paper>
            ...
            >
              <CardContent>
                <Typography variant="h6" sx={{ mb: 1, fontWeight: 600 }}>
                  <LockIcon sx={{ mr: 1, verticalAlign: 'middle', color: '#7c4dff' }} />
                  Step 1: Submit Sealed Bid
                </Typography>
                <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                  Enter your bid amount below. Your amount and salt stay on your local device. Only a ZK commitment hash is written to the ledger.
                </Typography>
          ...
          {auctionPhase === 'Reveal' && (
            ...
            >
              <CardContent>
                <Typography variant="h6" sx={{ mb: 1, fontWeight: 600 }}>
                  <VisibilityIcon sx={{ mr: 1, verticalAlign: 'middle', color: '#ff9100' }} />
                  Step 2: Reveal Your Bid
                </Typography>
                <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                  Click below to execute the ZK reveal circuit. If your bid exceeds the current highest bid ({highestBid.toString()} tNIGHT), it updates the on-chain ledger!
                </Typography>
                <Button
                  variant="contained"
                ...
                >
                  Reveal My Bid ({myBid ? myBid.amount.toString() : '10'} tNIGHT)
                </Button>
              </CardContent>
            </Card>
          )}
          ...
          {auctionPhase === 'Ended' && (
            ...
            >
              <CardContent>
                <Typography variant="h6" sx={{ mb: 1, fontWeight: 600, color: '#00e676' }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  The auction has ended. The winning bid was verified on-chain, and all un-winning bid amounts were 100% preserved in privacy.
                </Typography>
              </CardContent>
            </Card>
          )}
