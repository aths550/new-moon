...
import { D as EncryptionSecretKey } from "./midnight_ledger_wasm-Dv3N6P-x.js";
...
var hasRepeatedCharacters = (password) => {
	...
	for (let i = 1; i < password.length; i++) if (password[i] === password[i - 1]) {
		consecutiveCount++;
...
var hasSequentialPattern = (password) => {
	...
	for (let i = 0; i <= lowerPassword.length - 4; i++) {
		...
		for (let j = 1; j < 4; j++) {
			...
			if (currentCode === prevCode + 1) ascendingCount++;
			else ascendingCount = 1;
			if (currentCode === prevCode - 1) descendingCount++;
			else descendingCount = 1;
			if (ascendingCount >= 4 || descendingCount >= 4) return true;
		}
	}
	return false;
};
...
var validatePassword = (password) => {
	if (!password) throw new PasswordValidationError("Password is required for private state encryption.\nPlease provide a password via privateStoragePasswordProvider in the configuration.", "missing");
	if (password.length < 16) throw new PasswordValidationError(`Password is shorter than 16 characters`, "too_short");
	if (hasRepeatedCharacters(password)) throw new PasswordValidationError(`Password contains too many repeated characters (more than 3 identical in a row)`, "repeated_characters");
	const characterClasses = countCharacterClasses(password);
	if (characterClasses < 3) throw new PasswordValidationError(`Password must contain at least 3 of: uppercase letters, lowercase letters, digits, special characters. Found: ${characterClasses}`, "insufficient_classes");
	if (hasSequentialPattern(password)) throw new PasswordValidationError("Password contains sequential patterns (e.g., '1234', 'abcd'). Use a more random password", "sequential_pattern");
};
...
function assertSemVer(version, label) {
	if (typeof version !== "string" || version.length === 0 || version.length > 255) throw new Error(`Invalid ${label}: ${JSON.stringify(version)}`);
	if (!SEMVER_PATTERN.test(version)) throw new Error(`Invalid ${label}: ${JSON.stringify(version)}`);
}
/**
...
//#endregion
export { parseHex as _, MIN_SEQUENTIAL_LENGTH as a, validatePassword as b, assertIsContractAddress as c, assertSemVer as d, assertUndefined as f, parseEncPublicKeyToHex as g, parseCoinPublicKeyToHex as h, MIN_PASSWORD_LENGTH as i, assertIsHex as l, isHex as m, MAX_SAFE_NAME_LENGTH as n, PasswordValidationError as o, fromHex as p, MIN_CHARACTER_CLASSES as r, assertDefined as s, MAX_CONSECUTIVE_REPEATED as t, assertSafeName as u, toHex as v, warnIfInsecureRemoteUrl as x, ttlOneHour as y };
